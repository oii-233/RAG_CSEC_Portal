...
**ዘብ AI** is a RAG-enabled student assistant built to help students quickly find institutional information, report incidents, and access campus resources via conversational AI.
...
Frontend will usually run at `http://localhost:5173` (Vite default) and the backend at `http://localhost:5000` (example). Update `services`/`authService.ts` or `NEXT_PUBLIC_API_URL` equivalents if needed.
...
- Keep changes minimal and focused when editing services or models that impact production data.

**License & Attribution**

- Add your preferred license file (`LICENSE`) if you intend to open-source.

---

If you want, I can:

- Expand this README with setup examples for Docker and deployment steps, or
- Add a short `docs/` folder with developer notes and architecture diagrams.
